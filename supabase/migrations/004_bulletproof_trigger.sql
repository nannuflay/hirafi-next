...
BEGIN
  ...
  IF v_role = 'vendor' THEN
    -- LOWER() + explicit CASE avoids enum cast exceptions entirely.
    -- 'appliancerepair' catches the legacy camelCase value from the UI.
    v_category := CASE LOWER(TRIM(COALESCE(new.raw_user_meta_data->>'category', '')))
      WHEN 'transport'        THEN 'transport'::service_category
      WHEN 'appliance_repair' THEN 'appliance_repair'::service_category
      WHEN 'appliancerepair'  THEN 'appliance_repair'::service_category
      WHEN 'plumbing'         THEN 'plumbing'::service_category
      WHEN 'electricity'      THEN 'electricity'::service_category
      WHEN 'carpentry'        THEN 'carpentry'::service_category
      WHEN 'painting'         THEN 'painting'::service_category
      WHEN 'cleaning'         THEN 'cleaning'::service_category
      ELSE                         'other'::service_category
    END;

    -- Guard against non-numeric rate values coming from the form.
    BEGIN
